...
const Navbar = () => {
  ...
  const navItems = [
    ...
    { label: "Desarrolladoras", href: "/promotores" },
      ...
      {mobileOpen && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="md:hidden bg-foreground/95 backdrop-blur-xl px-6 pb-6">
          <div className="flex flex-col gap-4">
            ...
            <Link to={isDeveloper ? "/developer/projects/new" : "/auth?role=developer"} className="bg-primary text-primary-foreground px-5 py-3 rounded-xl text-sm font-medium mt-2 block text-center" onClick={() => setMobileOpen(false)}>
              {t("nav.publishProject")}
            </Link>
          </div>
        </motion.div>
